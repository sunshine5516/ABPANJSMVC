...
namespace ABPARJS.Tests
{
    public abstract class ABPARJSTestBase : AbpIntegratedTestBase<ABPARJSTestModule>
    {
        ...
        protected void UsingDbContext(Action<ABPARJSDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ABPARJSDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ABPARJSDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ABPARJSDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ABPARJSDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABPARJSDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ABPARJSDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABPARJSDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ABPARJSDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ABPARJSDbContext, Task<T>> func)
        {
            T result;
